...
//        System.out.println("nameProducer: " + nameProducer);
        ...
        if (prodName == null || prodName.isEmpty() || image == null || nameProducer == null) {
            sendResponeText(response, "Hãy điền đầy đủ thông tin sản phẩm", HttpServletResponse.SC_BAD_REQUEST);
            return;
        }
        //Kiểm tra ảnh sản phẩm
        if (image.isEmpty() || image == null) {

            sendResponeText(response, "Hãy nhập hình ảnh sản phẩm", HttpServletResponse.SC_BAD_REQUEST);
            return;
        }
        //Kiểm tra mô tả
        if (describle.isEmpty() || describle == null) {

            sendResponeText(response, "Hãy nhập mô tả sản phẩm", HttpServletResponse.SC_BAD_REQUEST);
            return;
        }
        //Kiểm tra thông số kỹ thuật
        if (specifions.isEmpty() || specifions == null) {

            sendResponeText(response, "Hãy nhập thông số kỹ thuật", HttpServletResponse.SC_BAD_REQUEST);
            return;
        }
//        Kiểm tra tên nhà sản xuất
        if (nameProducer.isEmpty() || nameProducer == null) {

            sendResponeText(response, "Hãy chọn nhà sản xuất", HttpServletResponse.SC_BAD_REQUEST);
            return;
        }
        //Kiểm tra tên sản phẩm
        if (prodName.isEmpty() || prodName == null) {

            sendResponeText(response, "Hãy nhập tên sản phẩm", HttpServletResponse.SC_BAD_REQUEST);
            return;

            //Kiểm tra tên sản phẩm có trùng không
        } else if (ProductDAO.getInstance().isExistProdName(prodName)) {
            sendResponeText(response, "Tên sản phẩm đã tồn tại", HttpServletResponse.SC_BAD_REQUEST);
            return;
        }
        ...
        try {
            ...
            if (unitPrice <= 0) {
                sendResponeText(response, "Hãy nhập giá sản phẩm", HttpServletResponse.SC_BAD_REQUEST);
                return;
            }
            //Kiểm tra loại sản phẩm có hợp lệ không
            if (cateId <= 0) {
                sendResponeText(response, "Hãy chọn loại sản phẩm", HttpServletResponse.SC_BAD_REQUEST);
                return;
            }

        } catch (NumberFormatException e) {
            //Bắt lỗi khi không nhập giá
            sendResponeText(response, "Hãy nhập số", HttpServletResponse.SC_BAD_REQUEST);
            return;
        }
        try {
...
//            System.out.println(product); //In ra sản phẩm sau khi đã kiểm tra
            ...
            sendResponeText(response, "Thêm sản phẩm thành công!", HttpServletResponse.SC_OK);
        } catch (Exception e) {
